...
describe('isLightHour', () => {
  ...
  function mockHour(hour) {
    const d = new Date(`2024-06-10T${String(hour).padStart(2, '0')}:00:00`);
    globalThis.Date = class extends ORIGINAL_DATE {
      constructor(...args) { if (args.length) return super(...args); return d; }
...
describe('ThemeProvider', () => {
  ...
  it('persists theme config to localStorage', async () => {
    ...
    await user.click(screen.getByText('Set Dark'));
    const stored = JSON.parse(localStorage.getItem('nutwatch-theme'));
